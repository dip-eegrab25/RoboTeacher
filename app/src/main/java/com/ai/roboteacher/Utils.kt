...
import com.ai.roboteacher.activities.SampleActivity
...
//    }
    ...
    public fun generatePDF(c:Context,questionsList:java.util.ArrayList<String?>?):String? {
        ...
        for (q in questionsList) {
            ...
            var s = StringBuilder()
            var cLine:StringBuilder = StringBuilder()
            ...
            for (w in words) {
                ...
                if (s.isEmpty()) {
                    ...
                    canvas.drawText(w+" ",xPosition,yPosition,paint)
                    s.append(w+" ")

                } else {

                    var x = paint.measureText(s.toString())
                    var wLength = paint.measureText("$w ")

                    if (x+wLength>pageWidth-20) {
...
//                s.append(w+" ")
//
//                if (paint.measureText(s.toString())<pageWidth) {
//
//                    cLine.append(w+" ")
//
//                } else {
//
//                    lines.add(cLine.toString())
//                    cLine.clear()
//                    cLine.append(w+" ")
//                    s.clear()
//                    s.append(w+" ")
...
//                }
//
//                canvas.drawText(l,10f,yPosition,paint)
//                yPosition+=12
//            }
//
//            yPosition+=22
...
//            }
//
//            val lines = wrapText(question!!,paint,pageWidth-80)
//
//            for (line in lines) {
//
//                if (yPosition>pageHeight-60) {
...
//    }
    ...
    private fun parseBlocks(data: String): List<PdfBlock> {
        ...
        regex.findAll(data).forEach { m ->

            if (m.range.first > lastIndex) {
                blocks += PdfBlock.Text(data.substring(lastIndex, m.range.first))
            }

            when {
                m.value.startsWith("$") -> {
                    blocks += PdfBlock.Equation(m.value)
                }
                m.value.startsWith("|") -> {
                    val rows = m.value
                        .trim()
        ...
        if (lastIndex < data.length) {
            blocks += PdfBlock.Text(data.substring(lastIndex))
        }
    ...
    private fun ensurePage(
        ...
        if (state.y > pageHeight - 40) {
            ...
            state.page = pdf.startPage(newInfo)
...
//        var yPosition = 25f
    ...
    private fun checkPageAndPrint(text:String
                                  ...
                                  ,pageHeight: Int
                                  ,startX:Float
                                  ,startY:Float):PdfDocument.Page {

        var newPage:PdfDocument.Page = page
    ...
    ,pagewidth:Int,pageHeight:Int):Page {
        ...
        pdfModel.page++
//        pageInfo = PdfDocument.PageInfo.Builder(pagewidth,pageHeight,pageNumber).create()
        val mPage = pdfDocument.startPage(PdfDocument.PageInfo.Builder(pagewidth,pageHeight,pdfModel.page).create())
...
//            }
            ...
            for (text in rList) {
                ...
                if (y+rowHeight>=pageHeight-25) {
                    ...
                    pdfModel.page++
                    mPage = pdfDocument.startPage(PdfDocument.PageInfo.Builder(pagewidth,pageHeight,pdfModel.page).create())
                    canvas = mPage.canvas
...
//                pdfModel.p.y = startY
    ...
    private fun renderEquation(match:String,pdfDocument: PdfDocument,page: Page
                               ...
                               startX: Float,startY: Float) : Page {
        ...
        if (match.startsWith("$$")) {
            ...
            canvas.drawBitmap(bitmap,x,y-20,null)
    ...
    public fun generatePdfNew(c:Context,pair:Pair<String,StringBuilder>):String {
        ...
        var emptyLineRegex = Regex("^\\s+")

        val data:String = pair.second.toString().replace(regex,"")
            .replace(headerRegex,"")
            .replace(tableRegex,"")
            .replace("`","")
            .replace("-","")
                ...
                if (match.contains(Regex("\\|(.)+\\|",RegexOption.DOT_MATCHES_ALL))) {
                    ...
                    if (index!=-1) {
                        ...
                        for (l in lines) {
                            ...
                            Log.d(Utils::class.java.name, "Lines: "+l)
                            ...
                            val lineCounts = rList.map { wrapText(it, cellWidth, paint).size }
                            val maxLines = lineCounts.maxOrNull() ?: 1
                            val rowHeight = maxLines * lineSpacing + padding * 4
                            ...
                            for (text in rList) {
                                ...
                                if (emptyLineRegex.matches(text)) {
                                ...
                                for (line in lines) {
                                    ...
                                    if (line.contains(Regex("\\\$\\\$(.*?)\\\$\\\$|\\\$(.*?)\\\$"))) {
                                        ...
                                        canvas.drawBitmap(bitmap,x+padding,textY-14,null)
                    ...
                    if (index!=-1) {
                        ...
                        if (!text.equals("")) {
                            ...
                            for (l in lines) {
                                ...
                                if (tValue.trim().length >= 3) {

                                    Log.d(Utils::class.java.name, "True")
...
// Create bitmap
                        ...
                        if (tValue.trim().length >= 3) {
                            ...
                            startX = x1
                            startY+=drawable.intrinsicHeight+28f

                        } else {

                            canvas.drawBitmap(bitmap,startX,startY-20,null)

                            startX+=drawable.intrinsicWidth
                        }
    ...
    public fun generatePDF3(c:Context,pair:Pair<String,StringBuilder>):String? {
                ...
                if (eqData.contains("$")) {
                    ...
                    if (!textData.equals("")) {
                        ...
                        for (l in lines) {
                            ...
                            val words = l.trim().split(Regex("\\s+"))
...
//    }
    ...
    public fun generatePDF4(c:Context,d:String):String? {

        var regex:Regex = Regex("""\*+|#+""",RegexOption.DOT_MATCHES_ALL)
        val data:String = d.replace(regex,"")
...
//                    if (startX+drawable.intrinsicWidth>pLimit) {
//
//                        startX = x1
//                        startY+=25
//                    }
...
//        return null
    ...
    public fun generatePDF2(c:Context,d:String):String? {
...
//        return null
    ...
    public fun generatePDF(c:Context,d:String):String? {
...
// Create bitmap
                ...
                xPosition = 295f
                yPosition+=drawable.intrinsicHeight+25
...
//        return null
    ...
    public fun savePDF(c:Context,pdfDocument: PdfDocument):String {
    ...
    private fun wrapText(question:String,paint:Paint,maxWidth:Int):ArrayList<String> {
    ...
    public fun enterFullScreenMode(window: Window) {
