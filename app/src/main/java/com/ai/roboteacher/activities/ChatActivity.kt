...
import com.ai.roboteacher.Models.CurriculumData
...
//                    if (dataSet.get(position).renderedMarkDown!=null) {
//
//                        //var f:SpannableString = dataSet.get(position).renderedMarkDown as SpannableString
//
//                        markWon!!.setParsedMarkdown(tv,dataSet.get(position).renderedMarkDown!!)
//
//
//                        tv.setTextColor(
//                            ContextCompat.getColor(this@ChatActivity, R.color.yellow_panda_logo_color)
//                        )
//
//                        Log.d(TAG, "onBindViewHolder: ")
//
...
//            }
    ...
    fun highlightPageNumber(text: Spanned): SpannableString {
        val regex = Regex("\\[(\\d+)]$")   // detect [25] at end of line
        val spannable = SpannableString(text)

        val match = regex.find(text)
        if (match != null) {
            val start = match.range.first
            val end = match.range.last + 1 // +1 because range is inclusive

            spannable.setSpan(
                ForegroundColorSpan(Color.WHITE),  // 🔴 highlight color
                0,
                spannable.length,
                Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
            spannable.setSpan(
                ForegroundColorSpan(Color.BLUE),  // 🔴 highlight color
                start,
                end,
                Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
            ...
            spannable.setSpan(
                object : ClickableSpan() {
                    override fun onClick(widget: View) {
                        // Handle click → e.g., go to PDF page
                        OkHttpClientInstance.getInstance(object : OkHttpClientInstance.ResultReceiver{
                            override fun onResult(code: Int, response: okhttp3.Response?) {
                                ...
                                response?.let { resp->
                                    ...
                                    resp.body?.let { body-> 
                                        ...
                                        val gson = Gson()
                                        val pdfResp = gson.fromJson(body.string(),CurriculumData::class.java)

                                        Log.d(TAG, "onResult: "+pdfResp.data!!.list.get(0).url)
                        ...
                        OkHttpClientInstance.get("${OkHttpClientInstance.curriculumUrl}?class=${classId}&subject=${subject}")
                    }

                    override fun updateDrawState(ds: android.text.TextPaint) {
                        super.updateDrawState(ds)
                        ds.isUnderlineText = false // remove underline if you want
                        ds.color = Color.RED        // clickable text color
                    }
                },
                start,
                end,
...
//
    ...
    override fun onResultReceived(whisperResult: WhisperResult?) {
        ...
        runOnUiThread {
            ...
            chatEditText!!.append(whisperResult?.result) } //append the result
                ...
                runOnUiThread {
                    ...
                    if (customDialog == null) {
                        ...
                        customDialog = CustomDialogBox()
                            .buildDialog(this@ChatActivity, R.layout.layout_processing_audio)
                            .setSize(
                                (Utils.getScreenWidth(this@ChatActivity) * 0.60).toInt(),
                                (Utils.getScreenHeight(this@ChatActivity) * 0.30).toInt()
                            )
                            .setCancelable(false)
                            .createDialog()
