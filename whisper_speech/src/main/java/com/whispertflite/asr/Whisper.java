...
//        threadProcessRecordBuffer.start();
    ...
    public void start(byte[] samples) {
...
//        }
    ...
    private void processRecordBuffer(byte[] samples) {

            try {

                if (mWhisperEngine.isInitialized() && samples != null) {
                    long startTime = System.currentTimeMillis();
                    sendUpdate(MSG_PROCESSING);

                    WhisperResult whisperResult = null;
                    synchronized (mWhisperEngine) {
                        whisperResult = mWhisperEngine.processRecordBuffer(mAction, mLangToken,samples);
                    }
